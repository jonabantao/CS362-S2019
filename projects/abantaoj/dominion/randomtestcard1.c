...
int testVillageCard(struct gameState*, int, int);
...
int main()
{
    ...
    for (int i = 0; i < TEST_COUNT; i++)
    {
        ...
        int result = testVillageCard(&testState, player, handPos);
...
int testVillageCard(struct gameState* state, int currPlayer, int handPos)
{
    int UNUSED_PARAM = 0;
    ...
    cardEffect(village, UNUSED_PARAM, UNUSED_PARAM, UNUSED_PARAM, state, handPos, &UNUSED_PARAM);
    ...
    if (prevActions != (state->numActions - 2))
    {
        hasPassedTests = 0;
    }
